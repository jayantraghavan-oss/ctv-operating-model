...
function SubModuleNode({
  ...
  return (
    <motion.button
      onClick={onClick}
      className={`relative text-left px-3 py-2.5 sm:py-2 rounded-xl border transition-all duration-300 group cursor-pointer
    ...
    >
      ...
      <div className="flex items-start justify-between gap-2">
        <span className="text-[12px] sm:text-[11px] font-semibold leading-tight line-clamp-2">{node.name}</span>
        <span className={`shrink-0 text-[10px] sm:text-[9px] font-bold px-1.5 py-0.5 rounded-md text-white ${colors.badge}`}>
          {colors.short}
        </span>
      </div>
      {node.promptCount > 0 && (
        <div className="mt-1 text-[10px] sm:text-[9px] opacity-60 font-medium">
  ...
  return (
    ...
    >
      ...
      <div className={`px-4 py-3 ${isC5 ? "" : "border-b " + colors.border}`}>
        <div className={`text-[10px] sm:text-[10px] font-bold uppercase tracking-widest ${isC5 ? "text-white/60" : colors.text} opacity-70`}>
          Cluster {cluster.id}
        </div>
        <div className={`text-[15px] sm:text-[14px] font-bold ${isC5 ? "text-white" : colors.text} leading-tight mt-0.5`}>
          {cluster.name}
        </div>
        {cluster.twoModes && (
        ...
        {sectionGroups.map((group, gi) => (
          <div key={group.key}>
            {!isC5 && sectionGroups.length > 1 && (
              <div className="text-[10px] sm:text-[9px] font-bold uppercase tracking-wider text-foreground/25 mb-1.5 px-1">
                {group.name}
              </div>
            )}
          ...
          >
            <div className="flex items-start gap-3 sm:gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[13px] sm:text-[13px] text-foreground/55 leading-relaxed">{s.description}</p>
              </div>
            </div>
        ...
        <div className="flex items-center justify-between mt-4 sm:mt-5 pt-3 sm:pt-4 border-t border-black/[0.05]">
          <div className="flex items-center gap-2 sm:gap-3">
            {step > 0 && (
              <button
                onClick={onPrev}
                className="flex items-center gap-1 text-[12px] font-medium text-foreground/40 hover:text-foreground/60 transition-colors py-1.5 px-2 -ml-2 rounded-lg active:bg-black/[0.04]"
              >
            ...
            <button
              onClick={onSkip}
              className="text-[12px] font-medium text-foreground/30 hover:text-foreground/50 transition-colors py-1.5 px-2 rounded-lg active:bg-black/[0.04]"
            >
              Skip
            </button>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            {isNavigateStep ? (
              <button
                ...
                className="flex items-center gap-2 px-4 sm:px-5 py-2.5 rounded-xl sm:rounded-2xl bg-primary text-white text-[13px] font-bold hover:bg-primary/90 transition-all shadow-lg shadow-primary/25 active:scale-95"
              >
              ...
              <button
                onClick={onNext}
                className="flex items-center gap-1.5 px-3.5 sm:px-4 py-2 rounded-xl text-[12px] font-semibold text-foreground/40 hover:text-foreground/60 border border-black/[0.08] hover:bg-black/[0.02] transition-all active:scale-95"
              >
              ...
              <button
                onClick={onNext}
                className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-primary text-white text-[13px] font-semibold hover:bg-primary/90 transition-all shadow-sm active:scale-95"
              >
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="chart">
            ...
            <motion.div
              initial={{ opacity: 0, y: 8 }}
              animate={{ opacity: 1, y: 0 }}
              className="flex items-center gap-3 sm:gap-4 mb-5 sm:mb-6 px-3 sm:px-4 py-2.5 sm:py-3 rounded-xl bg-black/[0.02] border border-black/[0.04] overflow-x-auto scrollbar-hide"
            >
              <span className="text-[10px] font-bold uppercase tracking-wider text-foreground/30 shrink-0">Legend</span>
              <div className="flex items-center gap-1.5 shrink-0">
                <div className="w-3 h-3 rounded-sm bg-blue-500" />
                <span className="text-[11px] text-foreground/50 font-medium whitespace-nowrap">AI (A)</span>
              </div>
              <div className="flex items-center gap-1.5 shrink-0">
                <div className="w-3 h-3 rounded-sm bg-amber-500" />
                <span className="text-[11px] text-foreground/50 font-medium whitespace-nowrap">AI+Review (A+H)</span>
              </div>
              <div className="flex items-center gap-1.5 shrink-0">
                <div className="w-3 h-3 rounded-sm bg-emerald-600" />
                <span className="text-[11px] text-foreground/50 font-medium whitespace-nowrap">Human (H)</span>
              </div>
              <div className="flex items-center gap-1.5 shrink-0">
            ...
            {/* Clusters 1-4 grid */}
            <div className={`grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3 sm:gap-4 transition-all duration-500 ${tourHighlight === "cluster-5" ? "opacity-40" : ""}`}>
              {[1, 2, 3, 4].map((cId) => (
                <ClusterCard
                  key={cId}
